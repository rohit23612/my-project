...
use fluXtore\Core\Admin\Query\HomeBoardQuery;
...
class HomeController extends \WC_REST_Reports_Controller {
	 ...
	 * @var string
	 */
	protected $rest_base = 'home/leaderboard';
	...
	public function get_items( $request ) {
		...
		$leaderboard_query = new HomeBoardQuery( $query_args );
	...
	public function get_leaderboard( $rows_data ) {
		...
		foreach ( $rows_data as $row ) {
			...
			$rows[]       = array(
				...
				array(
					'display' => $row['conversion_rate'] !== '-' ? ceil($row['conversion_rate'] * 100) . '%' : '-',
